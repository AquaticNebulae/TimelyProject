...
import { Search, Plus, Filter, ChevronRight, Phone, Users, Briefcase, Clock, X, Edit2, Trash2, User, TrendingUp, CheckCircle2, UserPlus, ArrowUpDown, ArrowUp, ArrowDown, StickyNote, FolderOpen, Link2, Unlink, UserMinus, FolderPlus, RefreshCw, Timer, Play, Pause, Save, CheckCircle, AlertCircle, Info } from 'lucide-react';
...
const ConsultantsPage = () => {
    ...
    const loadAllData = async () => { setRefreshing(true); await Promise.all([loadConsultants(), loadClients(), loadProjects(), loadClientConsultants()]); loadProjectConsultants(); loadHoursLogs(); setRefreshing(false); };
    const loadConsultants = async () => {
        ...
        if (d?.data) {
            // Merge API consultants with local consultants
            ...
            // API unavailable, use local consultants only
    ...
    const loadProjectConsultants = () => { try { const s = localStorage.getItem(STORAGE_KEYS.projectConsultants); if (s) setProjectConsultants(JSON.parse(s)); } catch (e) { console.error(e); } };
    ...
    const createConsultant = async () => {
        ...
        try {
            ...
            // Fallback: Create consultant locally when API unavailable
    ...
    const deleteConsultant = async (cid: string) => {
        try {
            ...
            // Even if API fails, clean up local data
            if (!response.ok) {
        ...
        // Always clean up local storage data
        const ext = JSON.parse(localStorage.getItem(STORAGE_KEYS.consultantsExtended) || '{}');
        delete ext[cid];
        localStorage.setItem(STORAGE_KEYS.consultantsExtended, JSON.stringify(ext));

        // Remove from project assignments
        saveProjectConsultants(projectConsultants.filter(pc => pc.consultantId !== cid));

        // Remove hours logs
        saveHoursLogs(hoursLogs.filter(h => h.consultantId !== cid));

        // Remove from client-consultant assignments (local)
        setClientConsultants(clientConsultants.filter(cc => cc.consultantId !== cid));
    ...
    const assignClient = async (clientId: string) => {
        if (!selectedConsultant) return;
        try {
            const r = await fetch(`${API_BASE}/client-consultants/assign`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clientId, consultantId: selectedConsultant.consultantId, performedBy: 'admin' }) });
            if (!r.ok) throw new Error('Failed');
            loadClientConsultants();
            showToast('Client assigned', 'success');
        } catch (e) { showToast('Failed to assign client', 'error'); }
    };

    const removeClient = (clientId: string) => {
        if (!selectedConsultant) return;
        setClientConsultants(clientConsultants.filter(cc => !(cc.consultantId === selectedConsultant.consultantId && cc.clientId === clientId)));
        showToast('Client removed', 'success');
    };

    const getConsultantClients = (cid: string) => clientConsultants.filter(cc => cc.consultantId === cid).map(a => clients.find(c => c.customerId === a.clientId)).filter(Boolean) as Client[];
    const getAvailableClients = (cid: string) => { const assigned = clientConsultants.filter(cc => cc.consultantId === cid).map(cc => cc.clientId); return clients.filter(c => !assigned.includes(c.customerId)); };

    const assignProject = (pid: string) => {
        if (!selectedConsultant) return;
        if (projectConsultants.find(pc => pc.consultantId === selectedConsultant.consultantId && pc.projectId === pid)) {
            showToast('Already assigned to this project', 'info'); return;
        }
        saveProjectConsultants([...projectConsultants, { projectId: pid, consultantId: selectedConsultant.consultantId, createdAt: new Date().toISOString() }]);
        showToast('Project assigned', 'success');
    };

    const removeProject = (pid: string) => {
        if (!selectedConsultant) return;
        saveProjectConsultants(projectConsultants.filter(pc => !(pc.consultantId === selectedConsultant.consultantId && pc.projectId === pid)));
        showToast('Project removed', 'success');
    };

    const getConsultantProjects = (cid: string) => projectConsultants.filter(pc => pc.consultantId === cid).map(a => projects.find(p => p.projectId === a.projectId)).filter(Boolean) as Project[];
    const getAvailableProjects = (cid: string) => { const assigned = projectConsultants.filter(pc => pc.consultantId === cid).map(pc => pc.projectId); return projects.filter(p => !assigned.includes(p.projectId)); };
    ...
    const startTimer = (pid: string) => { setTimerProjectId(pid); setTimerSeconds(0); setIsTimerRunning(true); showToast('Timer started', 'info'); };
    const stopTimer = () => {
    ...
    const filteredConsultants = useMemo(() => {
        ...
        return filtered;
    }, [consultants, searchTerm, statusFilter, sortConfig]);

    const stats = useMemo(() => ({ total: consultants.length, active: consultants.filter(c => c.status === 'active').length, onLeave: consultants.filter(c => c.status === 'on_leave').length, totalHours: hoursLogs.reduce((sum, h) => sum + h.hours, 0) }), [consultants, hoursLogs]);
    ...
    return (
        <div className={`min-h-screen ${styles.bg}`}>
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
                    <div className={`${styles.modal} border rounded-lg max-w-md w-full p-6`}>
                        ...
                        <p className={`${styles.textMuted} mb-6`}>This will permanently delete the consultant and all their assignments, time logs, and data. This action cannot be undone.</p>
            ...
            <div className="max-w-7xl mx-auto px-6 py-8">
                <div className="mb-8">
                    <div className="flex items-start justify-between mb-6">
                        <div><h1 className={`text-4xl font-bold ${styles.text} mb-2`}>Consultants</h1><p className={styles.textMuted}>Manage team and track hours</p></div>
                    ...
                    <div className="grid grid-cols-4 gap-4 mt-6">
                        {[{ label: 'Total', value: stats.total, icon: Users }, { label: 'Active', value: stats.active, icon: CheckCircle2 }, { label: 'On Leave', value: stats.onLeave, icon: Clock }, { label: 'Hours Logged', value: formatHours(stats.totalHours), icon: TrendingUp }].map((s, i) => (
                ...
                <div className={`${styles.card} border rounded-lg overflow-hidden`}>
                    ...
                    {filteredConsultants.length === 0 ? (
                        <div className="text-center py-16"><Users className={`w-12 h-12 ${styles.textSubtle} mx-auto mb-3`} /><p className={styles.textMuted}>No consultants found</p></div>
                    ) : filteredConsultants.map(c => (
                        <div key={c.consultantId} className={`grid grid-cols-12 gap-4 p-4 border-b ${styles.tableRow} cursor-pointer`} onClick={() => openConsultantDetails(c)}>
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
                    <div className={`${styles.modal} border rounded-lg max-w-3xl w-full max-h-[85vh] overflow-y-auto`}>
                        <div className={`p-5 border-b ${styles.divider} flex items-center justify-between sticky top-0 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
                            ...
                            <div className="flex items-center gap-2"><button onClick={() => setShowDeleteConfirm(selectedConsultant.consultantId)} className="p-2 hover:bg-red-500/20 rounded-lg"><Trash2 className="w-5 h-5 text-red-500" /></button><button onClick={() => { setShowDetailsModal(false); setSelectedConsultant(null); }} className={`p-2 ${styles.cardHover} rounded-lg`}><X className={`w-5 h-5 ${styles.textMuted}`} /></button></div>
                        </div>
                        <div className="p-5 space-y-5">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                ...
                                <button onClick={() => { setShowDetailsModal(false); setShowClientsModal(true); }} className={`flex items-center justify-center gap-2 px-4 py-2.5 ${styles.button} rounded-lg`}><Users className="w-4 h-4" />Clients</button>
                                ...
                                <button onClick={() => { setShowDetailsModal(false); setShowTimeTrackingModal(true); }} className={`flex items-center justify-center gap-2 px-4 py-2.5 ${styles.buttonPrimary} rounded-lg`}><Clock className="w-4 h-4" />Log Time</button>
                            </div>
            ...
            {showClientsModal && selectedConsultant && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
            ...
            {showProjectsModal && selectedConsultant && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
                    <div className={`${styles.modal} border rounded-lg max-w-xl w-full max-h-[85vh] overflow-y-auto`}>
            ...
            {showTimeTrackingModal && selectedConsultant && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
                    <div className={`${styles.modal} border rounded-lg max-w-2xl w-full max-h-[85vh] overflow-y-auto`}>
