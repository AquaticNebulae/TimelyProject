...
interface HoursLog { logId: string; projectId: string; consultantId: string; date: string; hours: number; description: string; createdAt: string; }
...
const HoursPage: React.FC = () => {
    ...
    // Timer functions
    const startTimer = () => { if (!timerProject) { showToast('Please select a project first', 'error'); return; } setIsTimerRunning(true); showToast('Timer started', 'info'); };
    const pauseTimer = () => { setIsTimerRunning(false); showToast('Timer paused', 'info'); };
    const stopTimer = () => {
        if (timerSeconds > 0) {
            const hours = Math.floor(timerSeconds / 3600);
            const minutes = Math.floor((timerSeconds % 3600) / 60);
            setLogForm({ ...logForm, projectId: timerProject, hours: hours.toString(), minutes: minutes.toString(), description: timerDescription, date: new Date().toISOString().split('T')[0] });
    ...
    const createHoursLog = async () => {
        ...
        try {
            const r = await fetch(`${API_BASE}/hours-logs`, {
                ...
                body: JSON.stringify({ projectId: logForm.projectId, consultantId: logForm.consultantId, date: logForm.date, hours: totalHours.toFixed(2), description: logForm.description, performedBy: 'admin' })
            });
            ...
            const newLog: HoursLog = { logId: `log_${Date.now()}`, projectId: logForm.projectId, consultantId: logForm.consultantId, date: logForm.date, hours: totalHours, description: logForm.description, createdAt: new Date().toISOString() };
    ...
    const resetLogForm = () => { setLogForm({ projectId: '', consultantId: '', date: new Date().toISOString().split('T')[0], hours: '', minutes: '', description: '' }); };
    ...
    const filteredLogs = useMemo(() => {
        return hoursLogs.filter(log => {
            const matchSearch = log.description.toLowerCase().includes(searchTerm.toLowerCase()) || getProjectName(log.projectId).toLowerCase().includes(searchTerm.toLowerCase()) || getConsultantName(log.consultantId).toLowerCase().includes(searchTerm.toLowerCase());
    ...
    }, [hoursLogs, searchTerm, filterProject, filterConsultant, projects, consultants]);

    const stats = useMemo(() => {
        const today = new Date().toISOString().split('T')[0];
        const weekDates = getWeekDates(currentWeekStart);
        const todayHours = hoursLogs.filter(l => l.date === today).reduce((sum, l) => sum + l.hours, 0);
        const weekHours = hoursLogs.filter(l => weekDates.includes(l.date)).reduce((sum, l) => sum + l.hours, 0);
        const totalHours = hoursLogs.reduce((sum, l) => sum + l.hours, 0);
        const uniqueProjects = new Set(hoursLogs.map(l => l.projectId)).size;
        return { todayHours, weekHours, totalHours, uniqueProjects };
    }, [hoursLogs, currentWeekStart]);
            ...
            <div className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="mb-8">
                    <div className="flex items-start justify-between mb-6">
                        <div><h1 className={`text-4xl font-bold ${styles.text} mb-2`}>Hours Tracker</h1><p className={styles.textMuted}>Log and track work hours by project</p></div>
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-6">
                        {[{ label: 'Today', value: formatHours(stats.todayHours), icon: Clock }, { label: 'This Week', value: formatHours(stats.weekHours), icon: Calendar }, { label: 'Total Hours', value: formatHours(stats.totalHours), icon: TrendingUp }, { label: 'Projects', value: stats.uniqueProjects.toString(), icon: FolderOpen }].map((s, i) => (
                ...
                {viewMode === 'list' ? (
                    <div className={`${styles.card} border rounded-lg overflow-hidden`}>
                        <div className={`grid grid-cols-12 gap-4 p-4 border-b ${styles.divider} ${styles.tableHeader}`}>
                            ...
                            <div className={`col-span-2 ${styles.textMuted} text-sm`}>Consultant</div>
                            <div className={`col-span-1 ${styles.textMuted} text-sm`}>Hours</div>
                            <div className={`col-span-3 ${styles.textMuted} text-sm`}>Description</div>
                            <div className={`col-span-1 ${styles.textMuted} text-sm text-right`}>Actions</div>
                        </div>
                        {filteredLogs.length === 0 ? (
                            <div className="text-center py-16"><Clock className={`w-12 h-12 ${styles.textSubtle} mx-auto mb-3`} /><p className={styles.textMuted}>No hours logged yet</p></div>
                            ...
                            <div key={log.logId} className={`grid grid-cols-12 gap-4 p-4 border-b ${styles.tableRow}`}>
                                ...
                                <div className="col-span-2 flex items-center gap-2"><User className={`w-4 h-4 ${styles.textSubtle}`} /><span className={styles.textMuted}>{getConsultantName(log.consultantId)}</span></div>
                                <div className="col-span-1"><span className={`${styles.accent} font-semibold`}>{formatHours(log.hours)}</span></div>
                                <div className="col-span-3"><p className={`${styles.textMuted} text-sm truncate`}>{log.description || '-'}</p></div>
                                <div className="col-span-1 flex items-center justify-end gap-1">
                                    ...
                                    <button onClick={() => setShowDeleteConfirm(log.logId)} className="p-2 hover:bg-red-500/20 rounded-lg"><Trash2 className="w-4 h-4 text-red-500" /></button>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
                    <div className={`${styles.modal} border rounded-lg max-w-lg w-full`}>
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div className={`${styles.cardInner} rounded-lg p-4`}><p className={`${styles.textSubtle} text-xs mb-1`}>Consultant</p><p className={styles.text}>{getConsultantName(selectedLog.consultantId)}</p></div>
                            ...
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => { setShowDeleteConfirm(selectedLog.logId); setShowEditModal(false); }} className="flex-1 px-5 py-2.5 bg-red-500/20 text-red-500 rounded-lg hover:bg-red-500/30">Delete</button>
