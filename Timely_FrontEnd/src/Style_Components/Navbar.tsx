...
type Props = {
  ...
  userName?: string;
  userEmail?: string;
  userRole?: UserRole; // ✅ NEW (for role-aware behavior if needed)
};
...
const Navbar: React.FC<Props> = ({
  ...
  const [searchResults, setSearchResults] = useState<
    { type: string; id: string; name: string; subtitle: string }[]
  >([]);
    ...
    const loadNotifications = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:4000/api/audit-logs/latest?limit=5"
        );
    ...
    const loadProjects = async () => {
      try {
        ...
        const localProjects = JSON.parse(
          localStorage.getItem("timely_projects") || "[]"
        );
        if (data.data) {
  ...
  // Role-aware allowed pages (Navbar is only used for staff, but we still define this)
  const safeRole: UserRole =
    userRole === "admin" || userRole === "consultant" || userRole === "client"
      ? userRole
      : "admin";
  ...
  const ROLE_PAGES: Record<UserRole, string[]> = {
    ...
    // client never uses this Navbar in our layout, so leave empty
    client: [],
  };

  const allowedPageIds = new Set(ROLE_PAGES[safeRole]);
  ...
  useEffect(() => {
    ...
    // Search pages - filter by allowed pages for this role
    const pages = [
      ...
      { name: "Admin Panel", id: "admin", subtitle: "System administration" },
    ];

    pages.forEach((p) => {
      if (!allowedPageIds.has(p.id)) return;
  ...
  }, [globalSearchQuery, projects, consultants, clients, allowedPageIds]);
  ...
  return (
    <header
      className={`fixed top-0 right-0 z-30 ${s.header} border-b transition-all duration-300 ${
        sidebarToggle ? "left-0" : "left-72"
      }`}
    >
      <nav className="flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-4">
          <button
            className={`p-2 ${s.button} rounded-lg transition-colors`}
          ...
          <div className="flex items-center gap-2 text-sm">
            <button
              ...
              className={`${s.textMuted} hover:text-blue-500 transition-colors`}
            >
              ...
              <>
                ...
                <div className="relative" ref={projectSelectorRef}>
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 ${s.button} rounded-lg transition-colors border ${s.divider}`}
                  >
                  ...
                  {showProjectSelector && (
                    <div
                      className={`absolute top-full left-0 mt-2 w-72 ${s.dropdown} border rounded-xl overflow-hidden z-50`}
                    >
                      <div className={`p-3 border-b ${s.divider}`}>
                        <div className="relative">
                          <Search
                            className={`absolute left-3 top-1/2 -translate-y-1/2 ${s.textMuted} w-4 h-4`}
                          />
                          <input
                            type="text"
                            ...
                            className={`w-full pl-10 pr-4 py-2 ${s.input} border rounded-lg text-sm focus:outline-none focus:border-blue-500`}
                          />
                        </div>
                      </div>
                          ...
                          filteredProjects.map((project) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <p
                                  className={`${s.text} text-sm font-medium truncate`}
                                >
                                  {project.projectName}
                                </p>
                                <p className={`${s.textMuted} text-xs`}>
                                  {project.projectCode}
                                </p>
                              </div>
                            </button>
                      ...
                      <div className={`p-2 border-t ${s.divider}`}>
                        <button
                          ...
                          className={`w-full px-4 py-2 flex items-center gap-2 ${s.accent} ${s.accentBg} rounded-lg transition-colors text-sm font-medium`}
                        >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={searchContainerRef}>
            {showSearch ? (
              <div className="relative">
                <div
                  className={`flex items-center gap-2 ${s.dropdown} rounded-lg border px-3 py-2`}
                >
                  ...
                  <button
                    ...
                    className={`${s.textMuted} transition-colors`}
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                {searchResults.length > 0 && (
                  <div
                    className={`absolute top-full left-0 right-0 mt-2 ${s.dropdown} border rounded-xl overflow-hidden z-50`}
                  >
              ...
              <button
                ...
                className={`p-2.5 ${s.button} rounded-lg transition-colors`}
              >
          ...
          <div className="relative" ref={notificationRef}>
            <button
              ...
              className={`relative p-2.5 ${s.button} rounded-lg transition-colors`}
            >
            ...
            {showNotifications && (
              <div
                className={`absolute right-0 top-full mt-2 w-80 ${s.dropdown} border rounded-xl overflow-hidden z-50`}
              >
                <div
                  className={`px-4 py-3 border-b ${s.divider} flex items-center justify-between`}
                >
                  <h3 className={`${s.text} font-semibold`}>Notifications</h3>
                  {unreadNotifications > 0 && (
                    <button
                      onClick={markAllAsRead}
                      className={`${s.accent} text-sm font-medium`}
                    >
                      Mark all as read
                    </button>
                  )}
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifications.map((notification: any) => (
                    <div
                      key={notification.id}
                      className={`px-4 py-3 flex items-start gap-3 ${s.dropdownHover} transition-colors border-l-2 ${
                        notification.read
                          ? "border-transparent"
                          : "border-blue-500 " +
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-sm ${
                            notification.read ? s.textMuted : s.text
                          }`}
                        >
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className={`flex items-center gap-3 px-3 py-2 ${s.dropdownHover} rounded-lg transition-colors`}
            >
            ...
            {showUserMenu && (
              <div
                className={`absolute right-0 top-full mt-2 w-56 ${s.dropdown} border rounded-xl overflow-hidden z-50`}
              >
                ...
                <div className="py-2">
                  <button
                    ...
                    className={`w-full px-4 py-2.5 flex items-center gap-3 ${s.dropdownHover} ${s.textMuted} transition-colors`}
                  >
                ...
                <div className={`py-2 border-t ${s.divider}`}>
                  <button
                    onClick={() => {
                      setShowUserMenu(false);
                      if (onLogout) {
                        // ✅ use real logout handler from App
                        onLogout();
                      } else {
                        // fallback – navigate to "logout" if handler not provided
                        onNavigate?.("logout");
                      }
                    }}
