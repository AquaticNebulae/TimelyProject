...
const Sidebar: React.FC<Props> = ({
    ...
    const bottomItems = [
        { id: "settings", label: "Settings", icon: FaRegSun },
    ];
    ...
    const NavItem = ({
        ...
        return (
            ...
            >
                {isActive && (
                    <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 ${styles.activeBar} rounded-r-full`} />
                )}
                ...
                `}>
                    <div className={`
                        ...
                        ${isHovered && !isActive && !isLogout ? `${styles.menuIconHover} scale-105` : ''}
                    `}>
            ...
            <div className="relative h-full flex flex-col px-4 py-6">
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 px-2">
                        ...
                        {/* Logo - only show when no back button or on dashboard */}
                        {(!onBack || activePage === "dashboard") && (
                            <div className={`w-10 h-10 ${isDark ? 'bg-gradient-to-br from-cyan-500 to-blue-600 shadow-lg shadow-cyan-500/25' : 'bg-blue-600'} rounded-xl flex items-center justify-center`}>
                                <span className="text-white font-bold text-lg">T</span>
                            </div>
                        )}

                        <div>
                            <h1 className={`text-xl ${styles.text} font-bold tracking-tight`}>Timely</h1>
                ...
                <div className="flex-1 overflow-y-auto">
                    {/* Main Menu */}
                    <div className="mb-6">
                        <p className={`px-4 mb-3 text-xs font-semibold ${styles.textSubtle} uppercase tracking-wider`}>
                            Main Menu
                        </p>
                        <ul>
                            {menuItems.map((item) => (
                    ...
                    {isAdmin && (
                        <div className="mb-6">
                            <p className={`px-4 mb-3 text-xs font-semibold ${isDark ? 'text-amber-500/80' : 'text-amber-600'} uppercase tracking-wider`}>
                                Administration
                            </p>
                            <ul>
                                {adminItems.map((item) => (
                ...
                <div className="mt-auto">
                    ...
                    {/* User Card - Clickable to expand */}
                    <div
                        className={`mt-4 p-3 ${styles.card} rounded-xl border cursor-pointer transition-all`}
                        onClick={() => setShowProfileDetails(!showProfileDetails)}
                    >
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm ${userRole === "admin"
                            ...
                            <div className="flex-1 min-w-0">
                                <p className={`text-sm font-medium ${styles.text}`}>
                                    {userRole === "admin" ? "Admin" : userRole === "consultant" ? "Consultant" : "Client"}
                                </p>
                            ...
                            <FaChevronRight className={`w-3 h-3 ${styles.textSubtle} transition-transform duration-200 ${showProfileDetails ? 'rotate-90' : ''}`} />
                        </div>

                        {/* Expanded Profile Details - Only shows when clicked */}
                        {showProfileDetails && (
                            <div className={`mt-3 pt-3 border-t ${isDark ? 'border-slate-700/50' : 'border-gray-200'} space-y-2`}>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className={`text-sm ${styles.textMuted} truncate`}>{userEmail || "No email"}</p>
                                </div>
                            </div>
                        )}
                    </div>
